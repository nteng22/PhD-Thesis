#### Housekeeping ####
setwd("~/Dropbox/QIB/KELLY Study/Raw data/")
...
species <- read_tsv("combined_species.tsv")
...
samples <- samples %>%
  dplyr::rename(ID_Patient = "ID Patient", Stool_sample = "Stool Sample",
...
species_transposed <- t(species_transposed)
# Need to change to data frame to use downstream
species_transposed <- as.data.frame(species_transposed)

# Now switch the rownames to columns, by Stool Sample
# So we can then identify by stool samples ID
species_transposed <- rownames_to_column(species_transposed, var = "Stool_sample")

# remove "genus_frac" of the column
species_transposed$Stool_sample <- gsub("-species_frac", "", species_transposed$Stool_sample)
...
species_sample_merge <- full_join(samples, species_transposed, by = "Stool_sample")
species_sample_merge_inner <- inner_join(samples, species_transposed, by = "Stool_sample")  

#### Example for using filtering ####
# Use select, to specify what you want to analyse
filtered_samples <- species_sample_merge_inner %>% 
...
write_tsv(species_sample_merge_inner, "species_sample_merged_211126.tsv")

# Function to make long data
library("reshape2")

# Melt, melts a variable into the data set- making it long data vs wide
species_sample_long <- melt(species_sample_merge_inner, 
...
data <- dplyr::select(normalised_data,
                      c(Eggerthella.lenta:ncol(normalised_data)))
...
# Make a dataframe "top" that contains the top to least most abundant species
# Taking the total 'sum' of columns
# i.e. per species which ones have the most reads. 
...
# Make a dataframe "other" that contains all of the other species You can alter these as you want.
...
# Combine the top species and the other column and check the rows all still add up to 100.
...
speciesPalette <- c( Phocaeicola.vulgatus = "cadetblue1",
                   Faecalibacterium.prausnitzii = "red",
                   Phocaeicola.dorei = "purple",
                   Bacteroides.uniformis = "blue",
                   Prevotella.copri = "green3",
                   Bacteroides.fragilis = "yellow",
                   Parabacteroides.distasonis = "red4",
                   Bacteroides.xylanisolvens = "green",
                   Alistipes.onderdonkii = "orange",
                   Bacteroides.thetaiotaomicron = "pink",
                   Others = "grey")
...
ggplot(sample_data_long,
  ...
  labs(title="Ten most abundant species",
       x = "Stool Sample ID",
...
# Save plot
ggsave("KELLY Ten most abundant species stacked box plots.png", width = 200, height = 200, units = c("mm"), dpi = 400)
