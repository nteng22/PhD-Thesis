...
MCB_data <- read_csv("../KELLY clinical metadata data-genus.csv")
...
reads <- merged_data %>%
  select(Cloacibacillus:ncol(merged_data))
...
rowsums <- rowSums(normalised_data)
rowsums <- as.data.frame(rowsums)
rowsums
...
data_dist_rows <- vegdist(normalised_data, method = "bray")
...
pdf(file = "~/Dropbox/QIB/KELLY Study/AEs/Heatmap_AEsGut.pdf", width = 11, height = 8)
