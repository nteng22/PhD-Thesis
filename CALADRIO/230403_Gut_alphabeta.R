...
adonis2(abundance_table ~ CBR*tPFS, 
       data = metadata, permutations = 999,
       method = "bray")
...
# Change values to make the figure nicer
...
ggplot(data = plot.data, aes(x = NMDS1, y = NMDS2)) + 
  geom_point(aes(colour = tPFS, shape = tPFS, fill = tPFS),
             size = 3) + 
  stat_ellipse(aes(colour = tPFS)) +
  ...
  labs(title = "Gut microbiota genera by tPFS") +
  theme(#Set the title font size
    plot.title = element_text(size=8),
    #Set the legend title position
    legend.position = "bottom",
