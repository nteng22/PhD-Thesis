...
normalised_data <- read_csv("Saliva_data/KELLY_saliva_normalised_data_Genus+Species_data.csv")
...
metadata <- read_csv("Saliva_data/KELLY_Saliva_metadata.csv")
metadata <- metadata %>%
...
write_csv(data, "KELLY_saliva_normalised%_genus+species_with_metadata.csv")

#### Data selection species ####
data <- read_csv("Saliva_data/KELLY_saliva_normalised%_genus+species_with_metadata.csv") #%>%
...
ggplot(shannon_metadata, aes(x = CBR, y = shannon_diversity),
  ...
  labs(title="KELLY saliva Shannon species EoT") +
...
ggsave("KELLY_saliva Shannon CBR species EoT.png", width = 200, height = 200, units = c("mm"), dpi = 400)
...
ggplot(simpson_metadata, aes(x = PDL1, y = simpson_diversity),
  ...
  scale_fill_manual(values=c("white", "white", "white")) + # inner colour can change
  #Set the axis labels and title
  labs(title="KELLY saliva Simpson species") +
  theme(#Set the title font size
    plot.title = element_text(size=8),
    #Set the legend title position
    legend.position = "bottom",
...
ggsave("KELLY_saliva Simpson PDL1 species.png", width = 200, height = 200, units = c("mm"), dpi = 400)

ggplot(shannon_metadata, aes(x = NLR, y = shannon_diversity),
       fill = NLR, shape = NLR) +
...
ggsave("KELLY_saliva Shannon NLR species EoT.png", width = 200, height = 200, units = c("mm"), dpi = 400)

#### Adonis species ####
adonis(abundance_table ~ PDL1,
...
ggplot(data = plot.data, aes(x = NMDS1, y = NMDS2)) + 
  geom_point(aes(colour = NLR, shape = NLR, fill = NLR),
             size = 3) + 
  ...
  labs(title = "KELLY Saliva species NLR") +
  theme(#Set the title font size
    plot.title = element_text(size=8),
    #Set the legend title position
    legend.position = "bottom",
...
ggsave("KELLY saliva NMDS NLR species.png", width = 200, height = 200, units = c("mm"), dpi = 400)
